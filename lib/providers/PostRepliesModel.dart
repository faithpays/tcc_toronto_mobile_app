...
class PostRepliesModel with ChangeNotifier {
  ...
  Future<void> fetchComments() async {
    try {
      var data = {"id": 0, "comment": commentId};
      ...
      if (response.statusCode == 200) {
        ...
        List<Replies> comments = await compute(parseComments, res);
  ...
  Future<void> fetchMoreComments() async {
    try {
      var data = {"id": items[0].id, "comment": commentId};
      final response = await Utility.getDio().post(
        ApiUrl.loadpostreplies,
        data: jsonEncode({"data": data}),
      );
      ...
      if (response.statusCode == 200) {
        ...
        print(res);
        hasMoreComments = res["has_more"];
        List<Replies> articles = await compute(parseComments, res);
  ...
  static List<Replies> parseComments(dynamic res) {
